...
describe('Soul workbench descriptors', () => {
  it('ships HR as the first specialized workbench and leaves other Souls on fallback', () => {
    ...
    expect(findSoulWorkbenchForSoul('hr')?.id).toBe('hr-people-workbench')
    expect(hasSpecializedSoulWorkbench('hr')).toBe(true)
  ...
  it('keeps HR actions tied to available capability templates and reviewable artifacts', () => {
    const workbench = soulWorkbenchDescriptorSchema.parse(findSoulWorkbenchForSoul('hr'))
    const templateIds = new Set(BUILTIN_CAPABILITY_TEMPLATES.filter(template => template.soulId === 'hr').map(template => template.id))
