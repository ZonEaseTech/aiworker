  ...
  if (ws.data.role === undefined) {
    ...
    if (authResult.via === 'enroll-otp') {
      if (!ctx.pendingEnrollments) {
        ctx.logger.error('[gateway] /enroll-ws 收到 OTP submit 但 ctx.pendingEnrollments 未注入')
        ws.close(4500, 'enroll_unavailable')
        return
      }
      const enroll = frame.enroll!
      const submission = ctx.pendingEnrollments.submit({
      ...
      try {
        ...
        ctx.pendingEnrollments.removeByWs(ws)
...
function handleClose(
  ...
  if (ws.data.role === 'node-pending') {
    ...
    const entry = ctx.pendingEnrollments?.removeByWs(ws)
    if (entry) {
      ctx.persistence.recordAudit({
        actor: 'gateway',
