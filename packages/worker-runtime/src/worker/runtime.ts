...
export class LocalWorkerRuntime {
  ...
  private buildSessionPrompt(session: SessionRow, requestLabel: string, request: string, metadata: Record<string, unknown>): string {
    ...
    const lines = [
      ...
      '',
      'User-visible answer style:',
      '- Do not start with tool, file, or skill usage narration.',
      '- Forbidden visible openings include: 我会, 我先, 我将, 已确认, 使用 `skill`, 使用 `.',
      '- If the draft starts with process narration, rewrite the opening before sending.',
      '- If you use skills, AGENTS, knowledge, templates, or MCP internally, keep that process out of the opening and start with the answer, deliverable, assumptions, or blocking question.',
      '',
      ...mentions,
      ...(mentions.length > 0 ? [''] : []),
      requestLabel,
