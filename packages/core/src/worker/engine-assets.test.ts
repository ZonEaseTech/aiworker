...
describe('engine asset projection', () => {
  ...
  it('suppresses baseline assets when matching worker overlays are disabled', async () => {
    ...
    await expect(stat(path.join(workspaceRoot, '.agents', 'skills', 'aiworker-hr-candidate-profile', 'SKILL.md'))).rejects.toThrow()
    ...
    expect(receipt.projections).not.toContainEqual(expect.objectContaining({ target: '.agents/skills/aiworker-hr-candidate-profile/SKILL.md' }))
