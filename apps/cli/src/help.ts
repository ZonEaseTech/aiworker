...
const ROOT_WORKER_SUMMARIES: Record<(typeof ROOT_WORKER_COMMANDS)[number], string> = {
  ...
  'daemon start': '后台启动本地 worker daemon，复用 up/init/serve 路径',
  ...
  'case list': '列出本地 worker 的 Worker Case File 投影',
  'case rerun': '基于现有 proof-loop 规则显式重跑一个 case',
  'case show': '查看一个 task 的 Case File、Review Decision、Evidence、Risk 和 Lessons',
  ...
  'brain inbox propose': '从 task 的 Brain Engine lesson candidates 创建 pending admission proposals',
  'brain journal show': '只读查看一个 worker task 的 Brain Journal trace',
  'brain memories': '只读列出或搜索 runtime brain memory',
  'review list': '列出本地 worker run reviews（workbench 默认复盘入口）',
  'review promote': '把 review lesson candidates 提升为 pending durable-context proposals',
  'review rerun': '基于 review evidence 显式重跑一个 work order',
  'review show': '查看一个 run review、evidence、risk 和 lesson candidates',
  'brain skills': '只读列出 fallback brain skill 与 native executor skill 目标',
  'brain skills sync-native': '同步 AIWorker 管理的 native executor skill projection（默认 dry-run）',
  ...
  'pack list': '列出 OD-style worker pack（SKILL.md + DOMAIN.md + work-order templates）',
  'pack show': '查看某个 worker pack 的 skill/domain/work-order/artifact 信息',
  ...
  return [
    ...
    },
    {
      ...
      body: [
        ...
        '  aiworker run --message "..."             向本地 daemon 提交一次 work order',
        '  aiworker review list                     查看 run reviews',
        '  aiworker review promote <runId>          把 lesson candidates 提升为 pending proposals',
      ].join('\n'),
    },
    {
      title: '常用查看',
    ...
    },
    {
      ...
      body: [
        ...
        '  aiworker brain --help                    Brain secondary/admin 命令',
