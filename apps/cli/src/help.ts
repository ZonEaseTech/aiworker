...
const ROOT_WORKER_SUMMARIES: Record<(typeof ROOT_WORKER_COMMANDS)[number], string> = {
  ...
  'executor capability list': '只读列出 executor-native MCP / engine plugin / engine skill / policy 声明',
  'executor capability show': '查看单个 executor-native capability descriptor',
  'executor doctor': '验证 executor 原生能力 manifest 和 engine CLI 状态',
  'executor mcp add': '声明一个 executor 原生 MCP server',
  'executor mcp sync': '投影 executor MCP 声明到 engine 官方 project 配置',
  'executor select': '显式选择本地 worker 的 task executor',
