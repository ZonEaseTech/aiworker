 ...
 * stub runtime（含真实 `WorkerEventBus` + stub orchestrator + dispose），
 * 让被测代码在 ingest 之后直接 emit 终态事件。
 */
...
describe('runRun terminal event mapping (BUG-005)', () => {
  it('orchestrator.finished → exit 0', async () => {
    ...
    const code = await runRun({ message: 'hello', timeoutMs: 5_000 })
    expect(code).toBe(0)
  ...
  it('timeout 时退出 124', async () => {
    ...
    const code = await runRun({ message: 'hello', timeoutMs: 80 })
