...
/**
 * aiworker schedule-* 系列命令直接调用 in-process CronService（短命进程：boot →
 * 调用 → 退出），不经 HTTP；与 `aiworker config-show` / `aiworker config-set` 一致的
...
async function withCron<T>(fn: (cron: import('@zonease/aiworker-core').CronService) => Promise<T>): Promise<T> {
  ...
  const cron = new CronService({
    workerId: 'aiworker-cli',
    getOrchestrator: () => ({
      ingest: async () => {
        throw new Error('aiworker schedule CLI 不应触发 cron tick；请检查实现')
      },
    }),
  })
  try {
...
export async function runScheduleList(): Promise<number> {
  try {
    ...
    consola.error(`[aiworker schedule-list] failed: ${err instanceof Error ? err.message : String(err)}`)
    return 1
  }
}
...
export async function runScheduleAdd(opts: ScheduleAddOptions): Promise<number> {
  try {
    ...
    consola.error(`[aiworker schedule-add] failed: ${err instanceof Error ? err.message : String(err)}`)
    return 1
  }
}
...
export async function runScheduleRemove(jobId: string): Promise<number> {
  try {
    ...
    consola.error(`[aiworker schedule-remove] failed: ${err instanceof Error ? err.message : String(err)}`)
    return 1
  }
}
