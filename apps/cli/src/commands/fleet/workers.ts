...
export async function runWorkersLaunch(opts: WorkersLaunchOptions = {}): Promise<number> {
  try {
    ...
    // 新 worker 配对完成：保存 deviceToken + defaultWorkerId（与 aiworker fleet pair 一致）。
    const { patchOperatorState } = await import('../../operator/state')
...
export async function runWorkersRemove(workerId: string): Promise<number> {
  try {
    ...
    const { loadOperatorState, saveOperatorState } = await import('../../operator/state')
