...
/** `aiworker fleet config get <workerId>` — 透传到 worker 的 config.get。 */
export async function runConfigGet(workerId: string): Promise<number> {
  try {
    ...
    consola.error(`fleet config get 失败: ${err instanceof Error ? err.message : String(err)}`)
...
/**
 * `aiworker fleet config set <workerId> <json> --if-match <version>` — 更新 worker 配置。
...
export async function runConfigSet(opts: ConfigSetOptions): Promise<number> {
  ...
  try {
    ...
    consola.error(`fleet config set 参数不是合法 JSON: ${err instanceof Error ? err.message : String(err)}`)
    return 2
  }
  ...
  try {
    ...
    consola.error(`fleet config set 失败: ${err instanceof Error ? err.message : String(err)}`)
    return errorToExitCode(err)
  }
}
