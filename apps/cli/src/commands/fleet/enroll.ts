...
/**
 * `aiworker fleet enroll` —— PLAN-019 / FEAT-026 OTP-attended enrollment 三件套：
 *
 *   aiworker fleet enroll list                  → enroll.list
 *   aiworker fleet enroll approve <otp>         → enroll.approve
 *   aiworker fleet enroll reject  <otp>         → enroll.reject
 *
...
export async function runEnrollList(deps: EnrollDeps = {}): Promise<number> {
  ...
  try {
    ...
    consola.error(`fleet enroll list 失败: ${err instanceof Error ? err.message : String(err)}`)
...
export async function runEnrollApprove(otp: string, deps: EnrollDeps = {}): Promise<number> {
  ...
  try {
    ...
    consola.error(`fleet enroll approve 失败: ${err instanceof Error ? err.message : String(err)}`)
    return mapErrorToExitCode(err)
  }
}
...
export async function runEnrollReject(otp: string, deps: EnrollDeps = {}): Promise<number> {
  ...
  try {
    ...
    consola.error(`fleet enroll reject 失败: ${err instanceof Error ? err.message : String(err)}`)
    return mapErrorToExitCode(err)
  }
}
