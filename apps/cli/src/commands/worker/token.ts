...
/**
 * `aiworker token rotate` — mint a new bearer token, persist the encrypted copy
 ...
 * `aiworker serve` (or manager-side register) will pick it up via `loadOrMintIdentity`.
 */
export async function runTokenRotate(): Promise<number> {
  ...
  consola.success(`[aiworker token rotate] worker ${ctx.workerId} token rotated`)
