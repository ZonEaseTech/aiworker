...
describe('aiworker doctor', () => {
  it('validates a freshly initialized project without bootstrapping extra state', async () => {
    ...
    expect(doctor.output).toContain('Brain runtime: Project Brain files and native executor projections are checked here')
    expect(doctor.output).toContain('aiworker lessons promote <runId>')
