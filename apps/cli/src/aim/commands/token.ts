...
/**
 * `aim token rotate <workerId>` — 为目标 worker 轮换 deviceToken。旧 token 立即失效。
 *
 * 注意：这里轮换的是 worker 连 gateway 用的 deviceToken（node 端），**不是** aim 自身
...
export async function runTokenRotate(workerId: string): Promise<number> {
  try {
    ...
    consola.error(`token rotate 失败: ${err instanceof Error ? err.message : String(err)}`)
