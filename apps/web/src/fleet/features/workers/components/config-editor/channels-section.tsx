...
/**
 * React key for a channel row. Most fleets only bind each channel type once,
 * so the channel name is a stable key; for the rare case where the operator
 * adds two of the same type, we disambiguate by counting prior occurrences.
 */
function channelRowKey(
  channel: ChannelType,
  all: ChannelBinding[],
  index: number,
): string {
      ...
      <header className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Inbound message sources the worker exposes as webhooks. Add the shown webhook URL in the platform's settings.
          </p>
        </div>
    ...
    <li className="flex flex-col gap-3 rounded-md border bg-background p-4">
      ...
      {webhookUrl && (
        <div className="flex flex-col gap-1">
          <Label>Webhook URL (read-only — paste this into the platform)</Label>
