...
export const builtinTemplateCopy: Record<SupportedLocale, Record<string, BuiltinTemplateCopy>> = {
  'en': {
    ...
    'profile-update-proposal': templateCopy('Candidate Profile Draft', 'candidate profile draft', 'Draft a reviewable profile update proposal that can become the accepted candidate profile after human review.', ['Candidate materials', 'Accepted profile baseline', 'Open HR questions'], ['Accepted profile draft is explicit and reviewable.', 'Proposal language is not promoted into the official profile.', 'Evidence, risks, and missing facts remain visible.']),
    'lifecycle-next-step': templateCopy('Lifecycle Next Step', 'lifecycle-next-step', 'Prepare the next reviewable HR touchpoint for a person across recruiting, onboarding, employee care, or offboarding.', ['Person profile', 'Current lifecycle moment', 'Open questions'], ['Output cites the supplied context and labels missing evidence.', 'Next action is concrete, owned, and useful for a human reviewer.', 'Decision and employment commitments remain human-owned.']),
  ...
  'zh-CN': {
    ...
    'profile-update-proposal': templateCopy('候选人档案草案', '候选人档案草案', '生成可 review 的档案更新提案，人工接受后才会成为正式候选人档案。', ['候选人材料', '当前正式档案基线', '开放 HR 问题'], ['待接受档案草案必须明确且可 review。', '提案状态文案不会进入正式档案。', '证据、风险和缺失事实保持可见。']),
    'lifecycle-next-step': templateCopy('生命周期下一步', '生命周期下一步', '为招聘、入职、员工维护或离职场景准备下一次可 review 的 HR 触点。', ['人员档案', '当前生命周期节点', '开放问题'], ['输出引用给定上下文并标出缺失证据。', '下一步具体、有负责人且对人工评审有用。', '决策和雇佣承诺保持由人负责。']),
    ...
    'onboarding-plan': templateCopy('入职计划', '入职计划', '生成 30/60/90 入职或 check-in 计划，包含负责人、证据、风险和跟进问题。', ['员工档案', '岗位期望', '团队上下文'], ['输出引用给定上下文并标出缺失证据。', '下一步具体、有负责人且对人工评审有用。', '不写隐私性或无依据的雇佣判断。']),
    'offboarding-summary': templateCopy('离职交接总结', '离职交接总结', '准备离职或 alumni 交接总结，包含知识转移、开放动作和隐私护栏。', ['离职员工上下文', '交接记录', '开放动作'], ['输出引用给定上下文并标出缺失证据。', '下一步具体、有负责人且对人工评审有用。', '敏感细节最小化并经过 review。']),
    'evidence-matrix': templateCopy('证据矩阵', '证据矩阵', '按岗位信号横向比较候选人，标出有来源的覆盖、缺口和冲突。', ['岗位 rubric', '候选人证据包', '面试记录或评分表'], ['输出引用给定上下文并标出缺失证据。', '覆盖状态区分强、弱、缺失或冲突。', '没有人工 review 前不做候选人排序。']),
  ...
  'ja': {
    'profile-update-proposal': templateCopy('候補者プロフィール案', '候補者プロフィール案', '人間のレビュー後に受け入れ済みプロフィールになり得るレビュー可能なプロフィール更新案を作成します。', ['候補者資料', '受け入れ済みプロフィール', '未解決の HR 質問'], ['受け入れ候補のプロフィール案が明示的でレビュー可能。', '提案状態の文言を正式プロフィールに昇格しない。', '証拠、リスク、不足事実が見える。']),
    ...
    'evidence-matrix': templateCopy('エビデンスマトリクス', 'エビデンスマトリクス', '候補者を職務シグナルに対して比較し、根拠、ギャップ、矛盾を示します。', ['職務ルーブリック', '候補者証拠パケット', '面接メモまたはスコアカード'], ['提供されたコンテキストを引用し、不足証拠を示す。', 'カバレッジは強い、弱い、欠落、矛盾を区別する。', '人間のレビューなしに候補者を順位付けしない。']),
  ...
  'de': {
    'profile-update-proposal': templateCopy('Kandidatenprofil-Entwurf', 'Kandidatenprofil-Entwurf', 'Erstellt einen reviewbaren Profiländerungsvorschlag, der nach menschlicher Review zum akzeptierten Kandidatenprofil werden kann.', ['Kandidatenmaterial', 'Akzeptierte Profilbasis', 'Offene HR-Fragen'], ['Der akzeptierbare Profilentwurf ist explizit und reviewbar.', 'Vorschlagsstatus-Sprache wird nicht ins offizielle Profil übernommen.', 'Evidenz, Risiken und fehlende Fakten bleiben sichtbar.']),
