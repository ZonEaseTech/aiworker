 ...
 * 常量时间字符串比较。
 *
 * 与 `apps/api/src/dashboard/middleware/auth.ts` 及
 * `apps/api/src/worker/secrets/crypto.ts` 的实现语义一致：长度不等直接 false
 * （长度本身并非秘密，且是公开观察到的 token 形状），否则走
 * `node:crypto.timingSafeEqual`。
 *
 * 按 CLAUDE.md §"加密与认证" 的要求：bearer token 比较必须常量时间。gateway
 * 是新独立进程，为了避免跨 app 耦合，这段三行代码**有意复制**自 dashboard
 * middleware，与 CLAUDE.md §Architecture Constraints 关于 crypto 副本的
 * 取舍一致（manager / worker / gateway 各持一份，不抽取为共享模块）。
 */
