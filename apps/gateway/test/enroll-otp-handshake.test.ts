...
let pendingEnrollments: PendingEnrollmentRegistry
...
beforeEach(() => {
  ...
  pendingEnrollments = new PendingEnrollmentRegistry({ ttlMs: 60_000 })
  ctx = {
    persistence,
...
afterEach(() => {
  ...
  ctx.pendingEnrollments?.dispose()
  closeFleetDb()
...
describe('PLAN-019 S3 — /enroll-ws OTP submit', () => {
  test('1. happy path：推 enrollment.otp + 落 requested audit + role=node-pending + 不进 fleet', () => {
    ...
    expect(pendingEnrollments.list()).toHaveLength(1)
    expect(pendingEnrollments.list()[0]!.workerId).toBe(WORKER_ID)
  ...
  test('6. node-pending 期间 ws.close 但 entry 已被 approve/reject → 幂等不写 abandoned audit', () => {
    ...
    const list = pendingEnrollments.list()
    const otp = list[0]!.otp
    const entry = pendingEnrollments.approve(otp)
