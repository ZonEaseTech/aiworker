import type { StaticMessages } from '../types'

import { en } from './en'

export const zhCN = {
  accessibility: {
    artifactSettings: '打开输出设置',
    sessionOutputPreview: '会话输出预览',
    closeDialog: '关闭对话框',
    closeSettings: '关闭本地设置',
    collapseSessionDetail: '收起会话详情',
    expandSessionDetail: '展开会话详情',
    gridView: '网格视图',
    languageSwitcher: '工作区语言',
    listView: '列表视图',
    moreCreationOptions: '更多项目创建选项',
    openSettings: '打开本地设置',
    projectFilters: '项目筛选',
    refreshWorkspace: '刷新工作区',
    searchProjects: '搜索项目',
    selectedSoul: '已选 Soul',
    soulCatalog: 'Soul 目录',
    soulProjectCreator: 'Soul 项目创建器',
    soulProjectsAndArtifacts: '工作区与会话',
    viewMode: '视图模式',
    workspace: '工作区',
  },
  app: {
    brand: 'AIWorker',
    loading: '正在加载工作台...',
    subtitle: '工作区与会话',
    workspacePill: '工作台',
  },
  artifact: {
    defaultHint: '选择或创建工作区以查看会话输出。',
    empty: '会话运行后，输出会显示在这里。',
    label: '会话输出',
    loading: '正在加载会话输出...',
    noSession: '暂无会话',
    pending: '输出待生成',
  },
  common: {
    available: '可用',
    comingSoon: '即将推出',
    interface: '界面',
    installed: '已安装',
    notInstalled: '未安装',
    workspace: '工作区',
  },
  create: {
    sessionContext: '会话上下文',
    creatingSession: '正在创建会话...',
    footer: '会话默认保留在当前工作区内。',
    newProject: '新建 Soul 项目',
    projectName: '工作区名称',
    projectPlaceholders: {
      default: '结账发布检查清单',
    },
    submit: '创建工作区会话',
  },
  languageOptions: en.languageOptions,
  navigation: {
    createTabs: {
      project: '项目',
    },
    projectTabs: {
      recent: '最近',
      thisSoul: '当前 Soul',
    },
    topTabs: {
      artifacts: '输出',
      connectors: '连接器',
      domainSystems: '领域系统',
      examples: '示例',
      projects: '项目',
    },
  },
  projects: {
    empty: {
      detail: soulName => `创建一个 ${soulName} 工作区，开始工作。`,
      title: '还没有工作区',
    },
    searchPlaceholder: '搜索工作区...',
  },
  workspace: {
    artifactCount: count => `${count} 个输出`,
    byokNeedsKey: 'BYOK 需要提供方、模型和 API key 引用后才能发送消息。',
    byokReady: (provider, model) => `${provider} ${model} 已就绪，可以发送消息。`,
    composerReadyAction: '打开设置',
    copyAsMarkdown: '复制为 Markdown',
    enginePosture: 'AI 引擎以高权限运行：可读写本工作区的文件，并可能访问网络。',
    retryMessage: '重试',
    archive: '归档',
    archiveSession: '归档会话',
    archiveWorkspace: '归档工作区',
    archiveConfirmation: {
      cancel: '取消',
      session: {
        detail: sessionTitle => `“${sessionTitle}” 将离开活跃会话列表。`,
        title: '确认归档会话？',
      },
      workspace: {
        detail: workspaceName => `“${workspaceName}” 及其活跃会话将离开当前 worker。`,
        title: '确认归档工作区？',
      },
    },
    configure: '配置',
    continueSession: '继续会话',
    createSession: '创建会话',
    createSessionPlaceholder: '描述目标、上下文、输入材料或期望的会话输出...',
    createSessionPrompt: workspaceName => `要在 ${workspaceName} 中构建什么？`,
    addSourceMaterials: '添加输入材料',
    attachedSourceMaterials: '已附加输入材料',
    closeSourceMaterialPreview: '关闭输入材料预览',
    createWorkspace: '创建工作区',
    createWorkspaceHint: '创建归属当前 worker 的工作区；会话在工作区内创建。',
    appApiRoute: routePrefix => `API ${routePrefix}`,
    appPermissionSummary: (appId, count) => `${appId} · ${count} 个权限`,
    appRoute: (label, path) => `路由 ${label} · ${path}`,
    developerDetails: '开发者详情',
    engineLoading: '正在检查执行设置...',
    engineMissing: engineId => `本地设置中没有 ${engineId}。`,
    engineNotInstalled: engineName => `已选择 ${engineName}，但 PATH 中未安装。`,
    engineReadyDetail: engineName => `${engineName} 已就绪，可以发送消息。`,
    engineRole: 'AIWorker 引擎',
    engineStarting: '正在生成回复...',
    eventCount: count => `${count} 条事件`,
    eventStream: '会话事件',
    followUpInput: '继续对话',
    followUpPlaceholder: '让当前 Soul 继续工作、细化输出或补充来源上下文...',
    hideDeveloperDetails: '收起开发者详情',
    mountedSlotCount: count => `${count} 个挂载槽位`,
    noEvents: '发送消息后会显示事件。',
    noOtherWorkspaces: '暂无其他工作区。',
    noSelectionDetail: '创建或选择一个工作区，查看会话和会话工作。',
    noSelectionTitle: '未选择工作区',
    noInvocations: '此会话还没有消息。',
    sendInvocation: '发送',
    sendingInvocation: '正在发送...',
    stopGenerating: '停止',
    stopGeneratingAriaLabel: '停止生成',
    backToWorker: '返回 worker',
    backToWorkspace: '返回工作区',
    currentSession: '当前会话',
    currentWorker: '当前 worker',
    currentWorkspace: '当前工作区',
    newSession: '新建会话',
    newWorkspace: '新建工作区',
    noWorker: '暂无 worker',
    noWorkspaceSessions: '此工作区还没有会话。',
    otherWorkspaces: '其他工作区',
    selectedWorkspace: '已选工作区',
    sessionDetail: '会话',
    latest: '最新',
    materialReadError: '无法读取其中一个已附加文件。',
    soulCatalog: 'Soul 目录',
    previewSourceMaterial: name => `预览 ${name}`,
    removeSourceMaterial: name => `移除 ${name}`,
    invocationCount: count => `${count} 条消息`,
    invocationHistory: '消息历史',
    updated: when => `更新于 ${when}`,
    workspaceNavigation: '工作区导航',
    workspaceSessions: '工作区会话',
    workspaceKicker: 'WORKER 工作区',
    workspaceList: '工作区',
    workspaceTitle: soulName => `${soulName} 工作区`,
    workerEngine: '默认引擎',
    workerId: 'Worker ID',
    workerList: 'Worker 列表',
    workerListHint: '先选择 worker，再管理它的工作区。',
    workerSoul: 'Soul 绑定',
    workerStatus: 'Worker 状态',
  },
  chat: {
    preparingResponse: '正在准备回复',
    startingInvocation: '正在开始',
    waitingForEngine: '正在等待引擎响应。',
    timeline: {
      invocationStarted: '已开始',
      working: '处理中',
      invocationWarning: '警告',
      engineWarning: '引擎警告',
      invocationCompleted: '已完成',
      invocationCancelled: '已取消',
      invocationFailed: '失败',
      processStarted: '进程已启动',
      processExited: '进程已退出',
      processLost: '进程已丢失',
      engineError: '引擎错误',
    },
  },
  relativeTime: {
    daysAgo: days => `${days} 天前`,
    hoursAgo: hours => `${hours} 小时前`,
    minutesAgo: minutes => `${minutes} 分钟前`,
    now: '刚刚',
  },
  settings: {
    about: {
      executionMode: '执行模式',
      hint: '运行时详情来自工作区 daemon。',
      selectedEngine: '已选引擎',
      title: '本地工作区运行时',
      updated: '更新时间',
      version: '版本',
    },
    appearance: {
      dark: '深色',
      hint: '选择此工作区界面的呈现模式。',
      light: '浅色',
      system: '跟随系统',
      title: '外观',
    },
    autosave: {
      failed: '保存失败',
      saved: '所有更改已保存',
      saving: '正在保存',
    },
    byok: {
      apiKeyRef: 'API key 引用',
      baseUrl: 'Base URL',
      hint: 'API key 字段只保存引用。使用 env:NAME 从 daemon 环境解析密钥。',
      model: '模型',
      provider: '提供方',
      title: 'BYOK 提供方',
    },
    connectors: {
      configured: '已配置',
      hint: '仅在 Soul 需要从团队系统读取来源上下文时启用连接器。',
      notConfigured: '未配置',
      title: '连接器',
    },
    dialog: {
      kicker: '本地 AIWORKER',
      subtitle: '配置此工作区的会话执行、本地适配器、Soul、语言和外观。',
      title: '本地设置',
    },
    engine: {
      availableCount: count => `${count} 个可用`,
      hint: '安装状态来自工作区 daemon 的 PATH 扫描。发送消息需要配置外部引擎或 BYOK 提供方。',
      testing: '正在测试引擎...',
      test: '测试',
      rescan: '重新扫描',
      title: '本地 CLI 引擎',
    },
    externalMcp: {
      hint: 'Workspace MCP binding 尚未落地，这里不会 materialize 到 engine。',
      pending: 'Local MCP server 会在配置后通过 workspace binding 和 secret reference 启用。',
      placeholder: 'command --arg value',
      title: '外部 MCP 服务器',
    },
    language: {
      hint: '保存到工作区 daemon 的设置记录中。',
      title: '语言',
    },
    localMcp: {
      hint: '等待 workspace MCP binding；当前设置不会启动服务器。',
      pending: '尚未接入。未来由 workspace binding 决定哪些 session 暴露 MCP context。',
      title: 'AIWorker 工作区 MCP 服务器',
      toggle: '本地工作区 MCP',
    },
    nav: {
      about: '关于',
      aboutDetail: '运行时详情',
      appearance: '外观',
      appearanceDetail: '系统 / 浅色 / 深色',
      connectors: '连接器',
      connectorsDetail: '团队系统访问',
      execution: '执行',
      executionDetail: '本地 CLI / BYOK',
      externalMcp: '外部 MCP',
      externalMcpDetail: '额外证据工具',
      language: '语言',
      languageDetail: '界面语言',
      localMcp: '本地 MCP',
      localMcpDetail: '工作区上下文服务器',
      soulPacks: 'Souls',
      soulPacksDetail: '安装、启用、诊断',
    },
    soulPacks: {
      connectorStatus: (connectorId, status) => `${connectorId} · ${status}`,
      connectorsTitle: 'Connectors',
      archiveApp: name => `归档 ${name}`,
      disabledConnector: '未启用',
      empty: '暂无已安装的 Souls。',
      enableApp: name => `启用 ${name}`,
      enabledConnector: '已启用',
      hint: '已安装的 Souls 决定哪些垂直产品可以创建 worker 和 workspace。诊断信息放在这里，不进入 worker 左栏。',
      title: 'Souls',
      unavailableConnector: '不可用',
      updating: '更新中',
    },
  },
  statuses: {
    active: '活跃',
    archived: '已归档',
    cancelled: '已取消',
    deleted: '已删除',
    draft: '草稿',
    failed: '失败',
    lost: '失联',
    queued: '排队中',
    running: '运行中',
    starting: '启动中',
    succeeded: '成功',
  },
  workerConfig: {
    overlayCategories: {
      'skill': '技能',
      'mcp-client': 'MCP 客户端',
      'entry-file': '入口文件',
    },
    viewEdit: '查看 / 编辑',
    view: '查看',
    addSkill: '添加技能',
    addEntryFile: '添加入口文件',
    editorTitle: assetId => `内容 · ${assetId}`,
    sourceBaseline: '基线',
    sourceOverlay: '叠加',
    contentLabel: '内容',
    readonlyHint: 'MCP 内容仅供查看且已脱敏。',
    save: '保存',
    saving: '保存中…',
    resetToBaseline: '重置为基线',
    cancel: '取消',
    discardChanges: '放弃更改',
    unsavedChangesTitle: '放弃未保存的更改？',
    unsavedChangesDescription: '右侧内容编辑尚未保存。切换前要放弃这些更改吗？',
    addNamePlaceholder: '名称',
    addContentPlaceholder: '内容',
    add: '添加',
    loadFailed: '加载内容失败。',
    executionPanel: '执行设置',
    executionPanelDetail: '引擎、模式、BYOK',
    keyRefLiteralError: 'API key 字段必须填引用（如 env:NAME），不能填字面密钥。',
  },
} satisfies StaticMessages
