...
If the active user prompt explicitly authorizes subagents, use them by default
for independent exploration, implementation, or verification sidecars. Keep the
main agent on the critical path and integrate subagent findings before the final
response.
...
- If a subagent cannot be joined or closed, keep the objective active, report the
...
## Standing Objective Contract

This skill never adapts its behavior to the outer runner mechanism. Runner state
is external. The skill contract is always the same: advance the standing release
objective by one verified slice.

- Completing one slice is progress, not objective completion.
- Passing preflight is not objective completion.
- Context compaction, low confidence, a failing test, or a confusing code path is
  not a reason to stop the objective.
- Do not report the objective `complete` until the Exit Criteria are satisfied.
- Do not report the objective `blocked` for ordinary drift, failing tests, or
  unknown code. First rehydrate, shrink the slice, choose another slice, or fix
  the drift.
- Report the objective `blocked` only when the same blocker repeats for at least
  three consecutive invocations and no alternate slice can make meaningful
  progress.
- The `Next` label is the next bounded implementation target to execute, not a
  suggestion for the user to decide.

## Compaction Recovery

At the start of each invocation, and after any context compaction or resume,
...
Only end the standing release objective when all of these are true:
...
Use commits as the durable progress anchor for long-running release work.
...
- If a slice cannot be committed, keep the objective active and report the exact
...
1. State the slice objective and write scope in one or two sentences.
...
Next: one next bounded implementation target.
```
