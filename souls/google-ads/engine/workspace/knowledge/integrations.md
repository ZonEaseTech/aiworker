...
> **密钥红线**：以下任何工具的真实凭据（OAuth client secret / refresh token / developer token / API key / service-account JSON）**永不进入 descriptor、数据库、receipt、日志、诊断输出、OpenAPI 示例或 UI**。接入时凭据由 Paseo 环境/provider CLI/MCP 配置持有，AIWorker 只记录 secret reference，绝不复制密钥。
...
1. **Paseo 环境内 MCP 形式**：所有 live 能力以 MCP server 接入（codex / claude-code 各自 config），凭据写在目标员工环境，AIWorker 不读 Soul 私有模块、不解释域字段。
2. **只读优先**：Phase 2 先开只读（拉数据做诊断 / 复盘），写操作（改出价 / 加否定词 / 发布 RSA / 改 GBP）需显式审批，走 Paseo/provider CLI 的 approval。
3. **多客户隔离**：代运营经 MCC 管多客户，Phase 2 接入须按客户 customer-id / GBP location 隔离数据与操作，禁止跨客户串数据（见 playbook §8）。
4. **密钥隔离**：见顶部红线。descriptor 仍只含 `protocol / identity / workspaceTemplate` 资产束，无 api / capability。
