...
describe('check-web-ui-components audit output', () => {
  test('classifies framed shadcn surfaces and separates scoped layout from raw native classes', () => {
    ...
    expect(result.stdout).toContain('apps/worker-web/src/worker/studio/first-run-soul-app-home.tsx: card x1')
    expect(result.stdout).toContain('apps/worker-web/src/worker/studio/mounted-surface.tsx: alert x2')
    expect(result.stdout).toContain('apps/worker-web/src/worker/studio/workspace-fallback.tsx: input-frame x1')
    expect(result.stdout).toContain('apps/worker-web/src/worker/worker-configuration-dialog.tsx: slotless-native-class x18')
    expect(result.stdout).toContain('apps/worker-web/src/worker/worker-workbench-tree.tsx: scoped-native-layout x1')
