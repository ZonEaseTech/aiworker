...
describe('check-web-ui-components audit output', () => {
  test('classifies framed shadcn surfaces and separates scoped layout from raw native classes', () => {
    ...
    expect(result.stdout).toContain('apps/web/src/worker/worker-studio.tsx: alert x5')
    ...
    expect(result.stdout).toContain('apps/web/src/worker/session-detail.tsx: scoped-native-layout x2')
    expect(result.stdout).toContain('web UI raw native control classification (accepted, enforced):')
    expect(result.stdout).toContain('apps/web/src/worker/session-detail.tsx: as-child-button x1')
    expect(result.stdout).toContain('apps/web/src/worker/session-turn-composer.tsx: hidden-file-input x1')
