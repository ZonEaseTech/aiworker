...
- **测试服部署只允许"已发布 npm cli + Caddy 反代"两件事**（2026-04-27 决策，REFACTOR-004 跟踪迁移）。具体：测试服上 **不允许** git clone 源码、`docker compose pull` GHCR 镜像、本机 `bun build` / `tsc` 编译；唯一允许的安装路径是 `npm install -g @zonease/aiworker-cli@<version>` + `aiworker install systemd`；唯一允许的更新路径是 `npm install -g @zonease/aiworker-cli@latest && systemctl restart aiworker-gateway`；Caddy 反代是唯一例外（静态 yaml/Caddyfile，由人工维护）。`scripts/deploy.ts`（GHCR + docker compose 路径）与 `ops/compose/*.yml` 仅适用其他场景或保留为参考，**不再用于测试服**。
- 测试机（唯一当前 target）：aissh server id `<aissh-server-id-redacted>`（`aiwork`，`<test-server-ip-redacted>`）；公网入口 `https://gateway.example.test`（Cloudflare 橙云代理终止 TLS，回源走 HTTP :80）；Caddy 是纯 `:80 → 127.0.0.1:9218` 反代（FEAT-030 起 9218 取代 3000）；`AIWORKER_MASTER_KEY` 丢失=全部已注册 worker 失联，务必在组织密钥库备份。当前测试服 fleet 仍跑 PLAN-019 时部署的 `/opt/aiworker` 源码 systemd unit（监听 :3000，旧默认），REFACTOR-004 完成前不动；迁移完成后 `/opt/aiworker` 整目录回收。
...
- worker 进 fleet 的四条路径（`registered_workers.addedBy` 四态）：
  1. **手动 pair**：`aiworker pair --bootstrap-token wtk_...`（gateway HTTP 回拨 worker `/info` 校验 → 落 fleet.db），`addedBy='manual'`。
  2. **自动 launch**：`AIWORKER_GATEWAY_CAN_LAUNCH=true` 下 supervisor 从容器 stdout 抓 bootstrap log 自动 pair，`addedBy='launch-local'`。
  3. **自助 enroll**（PLAN-018 / FEAT-024）：worker env 同时设 `AIWORKER_GATEWAY_URL` + `AIWORKER_JOIN_TOKEN`，`aiworker serve` 用 outbound WS 拨 `/ws`，在第一帧 `connect.enroll` 块（`mode='join-token'`）里把 join token + 自身 apiToken + 可选 displayName 传给 gateway；gateway `authorizeConnection` self-enroll 分支验签 → `upsertEnrolledWorker`，`addedBy='self-enroll'`。outbound-only，worker 不需要 inbound 端口。
  4. **OTP-attended enroll**（PLAN-019 / FEAT-026）：worker env 设 `AIWORKER_GATEWAY_URL`（无 `AIWORKER_JOIN_TOKEN`，或显式 `AIWORKER_ENROLL_MODE=otp`），`aiworker serve` 用 outbound WS 拨 `/enroll-ws`（**不**复用 `/ws`），第一帧 `connect.enroll.mode='otp'` 不带 join token；gateway `authorizeConnection` enroll-otp 分支放行（**无 token**），交 `PendingEnrollmentRegistry.submit` 派 8 字符 OTP 回推 worker，`ws.data.role='node-pending'`（不进 NodeRegistry，不广播 worker.online）；operator 在 `/ws` 通道用 `aiworker enroll approve <otp>` / `reject <otp>` 决定去留，approve 时才 `upsertEnrolledWorker(addedBy='otp')` 落 fleet.db 并通过原 ws 推 `enrollment.approved` 事件。`addedBy='otp'`，worker 部署方完全不需要持有 fleet 凭证。
