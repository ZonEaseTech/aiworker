# PLAN-360 CLI 0.17.5 patch release
...
The current external release state is `@zonease/aiworker-cli@0.17.4` on npm
latest and GitHub Release `v0.17.4`. `main` contains three post-release commits:

- `4b875249 fix: 收敛面板切换图标语义`
- `d4630f58 refactor: 收敛 HR 档案工作台右栏`
- `04dce1cd docs: 明确外部 operator 能力边界`
...
Publish `@zonease/aiworker-cli@0.17.5` as a patch release carrying the HR panel
toggle icon fix, HR People Workbench right-panel convergence, and the external
operator boundary documentation update.

Execution steps:

1. Bump `apps/cli/package.json` from `0.17.4` to `0.17.5`.
2. Create `REL-044` / `PLAN-360` release tracking.
...
6. Push `main`, create and push annotated tag `v0.17.5`.
7. Monitor the tag-triggered GitHub Actions release workflow.
8. Verify:
   - `npm view @zonease/aiworker-cli version dist-tags --json`
   - `bunx @zonease/aiworker-cli@0.17.5 --version`
   - `gh release view v0.17.5 --repo ZonEaseTech/aiworker --json tagName,isDraft,isPrerelease,assets,url,publishedAt,targetCommitish`
   - a published-package Host Web/API + official app smoke.
9. Close `REL-044`, `PLAN-360`, index files and `docs/changelog.md` with
...
- If npm publishes `0.17.5` but post-release smoke fails, do not overwrite that
  version; record the regression and prepare a follow-up patch release.
- This release is intentionally narrow. Further HR workbench UX changes should
  ship in a separate patch release.
...
- `docs/task/REL-044.md`
- `docs/task/index.md`
- `docs/plan/PLAN-360.md`
- `docs/plan/index.md`
- `docs/changelog.md`
...
- 2026-05-18：开始 `0.17.5` patch release prep。
...
- 2026-05-18：release prep commit `c414172a`、tag `v0.17.5` 已推送；
  release workflow `26044002863` 与 main lint workflow `26043996324` 均成功。
- 2026-05-18：post-release verification 通过；npm latest 为 `0.17.5`，
  GitHub Release `v0.17.5` 为 non-draft / non-prerelease 且包含 8 个
...
- [x] `gh run watch 26044002863 --repo ZonEaseTech/aiworker --exit-status`
- [x] `npm view @zonease/aiworker-cli version dist-tags --json`
- [x] `bunx @zonease/aiworker-cli@0.17.5 --version`
- [x] `gh release view v0.17.5 --repo ZonEaseTech/aiworker --json tagName,isDraft,isPrerelease,assets,url,publishedAt,targetCommitish`
