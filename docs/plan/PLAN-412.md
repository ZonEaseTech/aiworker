...
2. Return `{ engineId, engines, executionMode, byok }` from persisted local settings, with BYOK reduced to a non-secret readiness projection.
...
- Passed: `curl -fsS http://127.0.0.1:9327/api/local/settings/engines | jq '{engineId, executionMode, engineCount: (.engines | length), byok}'`
