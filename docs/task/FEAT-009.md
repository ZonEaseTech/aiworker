...
| 2026-04-23 08:46 | `38da2f3-202604230841` | `38da2f3` | claude-opus-4-7 | PLAN-009 FEAT-019 model picker + FEAT-020 dual-tag image + FEAT-022 auth recipe hint. First build that publishes both `:<sha>` (slim) and `:<sha>-full` (claude-code + codex + gemini-cli + qwen-code baked via `npm i -g`). Dashboard stays on slim (manager doesn't need agentic CLIs); workers choose per compose. GHCR build 5m0s (first double-tag run, full layer cold). `/health` 200 ok on `gateway.example.test`. web tests 32→37 (+5 FEAT-019); api/shared unchanged. lint stays 0. |
| 2026-04-23 08:59 | `7928639-202604230852` | `7928639` | claude-opus-4-7 | PLAN-009 FEAT-021 cursor-agent baked into `-full` image. GHCR build 3m41s (slim layer cache hit, only full stage paid network). In-container `docker run --rm` smoke on `-full` confirmed all 5 CLIs runnable: claude `2.1.112` / codex `0.121.0` / gemini (DeprecationWarning only, runs) / qwen (same) / cursor-agent `2026.04.17-787b533`. Dashboard redeployed on slim for tag alignment; `/health` 200 ok. PLAN-009 closed. |
