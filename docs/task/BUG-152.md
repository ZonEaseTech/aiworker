...
The local CLI executor now has a named hard timeout, failed runtime turns persist
terminal session/turn/invocation/event state, and the mounted universal
workbench refreshes full selected session detail until terminal status. The
timeline suppresses stale running/requesting signals and duplicate timeout
errors.
